...
def deleteCat():
    ...
    if choice in vocab.keys():    #WORKS UP TO THE POINT OF SHOWING KEYS AND VALS WITHIN DICT NEEDS TWEAKING!!!!!
        choice==(vocab[choice])
        print (vocab[choice])
        option=input("to delete the entire dictionary click d, to delete items from within it click w")
        if option == "d":
            del vocab[choice]
        elif option == "w":
         choice2=input("select an item")
         del vocab[choice][choice2]           #this function now allows user to delete either entire dictionary or item from within dictionary
        print ("item removed")
